module.exports = {
    ...
    productList2: {
        query: `
        select t3.* t4.path 
        from (select t1.*, t2.cetegory1, t2.category2, t2.category3 from t_product t1, t_category t2 where t1.category_id = t2.id) t3 left join (select * from tb_image where type = 1) t4
    ...
    productMainImages: {
        query: `select * from tb_image where product_id = ? and type = 2`
    },
    productInsert: {
